...
const CATS: { value: TeamCategory; label: string }[] = [
  ...
  { value: "TURING TRIAL", label: "Turing Trial" },
  { value: "KILL CODE", label: "Kill Code" },
  { value: "SYNTHORA", label: "Synthora" },
  { value: "BINARY BLITZ", label: "Binary Blitz" },
  { value: "THINKVERSE", label: "Thinkverse" },
  { value: "CASE TACTICS", label: "Case Tactics" },
  { value: "CODE WHIRL", label: "Code Whirl" },
  { value: "CUISINE COSMOS", label: "Cuisine Cosmos" },
  { value: "DESIGNOPS", label: "DesignOps" },
  { value: "COLOSSAL-A-PITCH", label: "Colossal-A-Pitch" },
];
...
function Dashboard({ onLogout }: { onLogout: () => void }) {
  ...
  const [tab, setTab] = useState<"team" | "gallery" | "uploads">("team");
  ...
  return (
    <div className="pt-28 pb-24 px-6 md:px-10 max-w-6xl mx-auto">
      ...
      <div className="flex gap-2 mb-8 border-b border-white/5">
        {(["team", "gallery", "uploads"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
          ...
          >
            {t === "team" ? "Team & Staff" : t === "gallery" ? "Gallery" : "Uploads"}
          </button>
        ))}
      </div>

      {tab === "team" ? <TeamManager /> : tab === "gallery" ? <GalleryManager /> : <UploadedManager />}
    </div>
  );
}

function UploadedManager() {
  const qc = useQueryClient();
  const list = useServerFn(adminListUploads);
  const remove = useServerFn(adminDeleteUpload);
  const { data } = useQuery({
    queryKey: ["admin-uploads"],
    queryFn: () => list() as Promise<UploadedRow[]>,
  });

  async function deleteUpload(row: UploadedRow) {
    if (!confirm("Delete this uploaded file?")) return;
    await remove({ data: { id: row.id, key: row.key } });
  ...
  return (
    <div>
      <div className="mb-6 text-sm text-muted-foreground">
        Uploaded files are stored in the Supabase storage bucket and tracked in the uploaded_files table.
      </div>
      <div className="grid gap-4">
        {(data ?? []).map((row) => (
          <div key={row.id} className="glass-panel p-4 flex flex-col gap-3">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1">
                <p className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">
                  {row.folder.toUpperCase()} • {row.filename}
                </p>
                <a
                  href={row.public_url}
                  target="_blank"
              ...
              <button
                ...
                className="inline-flex items-center justify-center rounded-full px-3 py-2 text-[10px] font-mono uppercase tracking-widest text-destructive hover:bg-red-500/10"
              >
                Delete
              </button>
            </div>
            <div className="grid grid-cols-2 gap-3 text-xs text-muted-foreground">
              <div>
                <div className="uppercase tracking-widest">Storage key</div>
                <div className="break-all">{row.key}</div>
              </div>
              <div>
                <div className="uppercase tracking-widest">Uploaded</div>
              ...
              <div>
                <div className="uppercase tracking-widest">Size</div>
    ...
    try {
      await upsert({
        data: {
          ...
          name: form.name || "",
          role: form.role || null,
          category: (form.category || "TURING TRIAL") as TeamCategory,
  ...
  return (
    <div>
      <div className="flex justify-end mb-6">
        <button
          onClick={() =>
            setEditing({ category: "TURING TRIAL", sort_order: 0, published: true })
          }
